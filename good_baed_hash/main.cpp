...
struct PlateHasherAll {
    ...
    size_t operator()(const VehiclePlate &plate) const
    {
        auto  code_char = plate.GetLetters();
        int64_t x =  (code_char[0]-'A'*10'00'000) + ( (code_char[1]-'A') * 10'000)+ (code_char[2]-'A') * 10'00'00'000;
        uint64_t h2 = x + plate.GetDigits() + plate.GetRegion() * 10'000'000'000ULL ;
        return static_cast<size_t>(h2);
    }
};
