...
TreeNode<T> *next(TreeNode<T> *node)
{
    ...
    {
        TreeNode<T> *buff_begin = begin(node->right);
        return buff_begin;
    }

    TreeNode<T> *buff_parent = node->parent;
    if (buff_parent->right != node && buff_parent != nullptr)
    {
        return buff_parent;
    }
    if (buff_parent->parent != nullptr)
    {
        return buff_parent->parent;
    }
    return nullptr;
}
